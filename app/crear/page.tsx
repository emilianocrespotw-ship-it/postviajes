...
// ─── Modal upgrade ────────────────────────────────────────────────────────────
// ── Número de WhatsApp del admin para contacto (sin + ni espacios) ────────────
const ADMIN_WHATSAPP = '5491100000000'  // TODO: reemplazá con tu número real

function UpgradeModal({ onClose }: { onClose: () => void }) {
  const waMsg = encodeURIComponent(
    '¡Hola! Llegué al límite de 10 posts del plan gratuito de PostViajes. Me gustaría continuar usando la herramienta. ¿Cómo puedo pagar el Plan Pro?'
  )
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-8 w-full max-w-sm text-center">
        ...
        <h2 className="text-xl font-black text-[#111827] mb-2">Llegaste al límite del mes</h2>
        <p className="text-gray-500 text-sm mb-1 leading-relaxed">
          Usaste los <strong className="text-[#111827]">10 posts gratuitos</strong> de este mes.
        </p>
        <p className="text-gray-500 text-sm mb-6 leading-relaxed">
          El contador se reinicia el 1° de cada mes, o podés pasarte al plan Pro para posts ilimitados.
        </p>
        <a
          href={waUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl font-black text-white bg-[#25D366] hover:bg-[#1fb855] transition mb-3"
        >
